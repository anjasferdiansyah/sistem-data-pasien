...
<body class="min-h-screen bg-slate-100" x-data="{ sidebarOpen: false }">
    ...
    <aside class="sidebar fixed top-0 left-0 h-full w-[270px] z-50 flex flex-col transition-transform duration-300"
        ...
        <nav class="flex-1 px-4 mt-2 space-y-1">
            ...
            <a href="{{ route('data-pasien.index') }}" class="sidebar-link {{ request()->is('data-pasien*') ? 'active' : '' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="!mt-6">
                ...
                <form method="POST" action="{{ route('logout') }}" class="sidebar-link w-full text-left">
                    @csrf
            ...
            <div class="flex items-center justify-between px-4 sm:px-6 lg:px-8 h-16">
                ...
                <div class="flex items-center gap-3">
                    <div class="hidden sm:flex items-center gap-2 text-sm text-slate-500">
                        ...
                        <span>{{ now()->locale('id')->format('l, d F Y') }}</span>
                    </div>
